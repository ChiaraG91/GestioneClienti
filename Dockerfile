...
WORKDIR /src
COPY ["GestioneClienti.csproj", "."]
RUN dotnet restore "./GestioneClienti.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./GestioneClienti.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Questa fase viene usata per pubblicare il progetto di servizio da copiare nella fase finale
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./GestioneClienti.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "GestioneClienti.dll"]